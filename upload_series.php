...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = trim($_POST['status'] ?? 'Ongoing'); // Default to 'Ongoing' if not set
    ...
    if (empty($maincategory)) {
        $error_messages['maincategory'] = 'Main category is required.';
    }
    if (empty($subcategory)) {
        $error_messages['subcategory'] = 'Sub category is required.';
    }
    if (empty($language)) {
        $error_messages['language'] = 'Language is required.';
    }
    if (empty($release_date)) {
        $error_messages['release_date'] = 'Release date is required.';
    }
    if ($duration <= 0) {
        $error_messages['duration'] = 'Duration must be greater than 0.';
    }
    if (empty($age_rating)) {
        $error_messages['age_rating'] = 'Age Limit is required.';
    }
    if ($seasons < 1) {
        $error_messages['seasons'] = 'Number of seasons must be at least 1.';
    }
    if ($episodes < 1) {
        $error_messages['episodes'] = 'Episodes per season must be at least 1.';
    }
    if (empty($status)) {
    ...
    if (empty($error_messages)) {
        // Prepare and bind
        $stmt = $conn->prepare("INSERT INTO series (title, description, maincategory, subcategory, language, release_date, duration, age_rating, poster, seasons, episodes, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssdssiii", $title, $description, $maincategory, $subcategory, $language, $release_date, $duration, $age_rating, $poster, $seasons, $episodes, $status);
